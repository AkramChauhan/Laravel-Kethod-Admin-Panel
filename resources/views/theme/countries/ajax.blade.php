...
@endphp
   <table class="table table-hover">
        ...
        @foreach($data as $v)
            <tr class="row_{{ $v->id }}">
                <td>{{ ++$record_id }}</td>
                <td><img class="img img-thumbnail" width="40px" src="{{ $v->flag }}" alt="{{ $v->name }}">&nbsp;&nbsp;{{$v->name }} - {{$v->nativeName }}</td>
                <td>{{$v->currencies_name }} [{{$v->currencies_symbol }}-{{$v->currencies_code }}]</td>
                <td>{{$v->capital }}</td>
                <td>{{$v->region }} - {{$v->subregion }}</td>
                <td>{{$v->population }}</td>
            </tr>
        <?php $page_number++ ?>
